...
class Labyrinthe :
    ...
    def __nom_inverse(self, c) :
        '''Convertir les coordonnées str en i,j
        Exemple : self.__nom('3_2') retourne (3,2)
        '''
        return (int(c.split('_')[0]) , int(c.split('_')[1]))
    ...
    def afficher(self, path=[]):
        ...
        if len(path)>0 :
            plt.plot(self.__nom_inverse(path[-1])[0] + 0.5, self.__nom_inverse(path[-1])[1] + 0.5, marker="o", markersize=7, markerfacecolor="yellow")
            plt.plot(self.__nom_inverse(path[0])[0] + 0.5, self.__nom_inverse(path[0])[1] + 0.5, marker="o", markersize=7, markerfacecolor="green")
            for i in range(len(path) - 1) :
                x, x1 = self.__nom_inverse(path[i])
                y, y1 = self.__nom_inverse(path[i + 1])
                plt.plot([x + 0.5, y + 0.5], [x1 + 0.5, y1 + 0.5], 'r')
